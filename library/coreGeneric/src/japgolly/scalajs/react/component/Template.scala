...
object Template {
  ...
  abstract class MountedMapped[F[_], A[_], P2, S2, P1, S1, P0, S0]
    ...
    override def setState[G[_]](s: State, callback: => G[Unit])(implicit G: Dispatch[G]) =
      ft.transSync(from.modState(ls set s, callback))

    override def modState[G[_]](f: State => State, callback: => G[Unit])(implicit G: Dispatch[G]) =
      ft.transSync(from.modState(ls mod f, callback))

    override def modState[G[_]](f: (State, Props) => State, callback: => G[Unit])(implicit G: Dispatch[G]) =
      ft.transSync(from.modState((s1, p1) => ls.set(f(ls.get(s1), mp(p1)))(s1), callback))

    override def setStateOption[G[_]](o: Option[State], callback: => G[Unit])(implicit G: Dispatch[G]) =
      o.fold(ft.transSync(from.setStateOption(None, callback)))(setState(_, callback))

    override def modStateOption[G[_]](f: State => Option[State], callback: => G[Unit])(implicit G: Dispatch[G]) =
      ft.transSync(from.modStateOption(ls `modO` f, callback))

    override def modStateOption[G[_]](f: (State, Props) => Option[State], callback: => G[Unit])(implicit G: Dispatch[G]) =
