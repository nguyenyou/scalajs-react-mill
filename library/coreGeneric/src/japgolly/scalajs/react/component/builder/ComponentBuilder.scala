...
object ComponentBuilder {
  ...
  given defaultToNoState[P]: Conversion[Step1[P], Step2[P, Unit]] =
    _.stateless

  given defaultToNoBackend[X, P, S](using ev: X => Step2[P, S]): Conversion[X, Step3[P, S, Unit]] = {
    case x: Step2[P, S] @unchecked => x.noBackend
    case x => ev(x).noBackend
  }
  ...
  final class LastStep[P, C <: Children, S, B, US <: UpdateSnapshot](
    ...
    /** This is the end of the road for this component builder.
      ...
      */
    def build(using ctorType: CtorType.Summoner[Box[P], C]): Scala.Component[P, S, B, ctorType.CT] = {
