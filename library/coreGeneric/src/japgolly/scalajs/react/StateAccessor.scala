...
trait StateAccessorImplicits2 {
  protected def castW[I, F[_], A[_], S](w: Write[?, F, A, ?]) = w.asInstanceOf[Write[I, F, A, S]]
  protected def castR[I, F[_], S](w: Read[?, F, ?]) = w.asInstanceOf[Read[I, F, S]]
  ...
  private def newScalaLifecycleStateW[S]: WritePure[Lifecycle.StateW[?, S, ?], S] = {
  ...
  implicit def scalaLifecycleStateW[S]: WritePure[Lifecycle.StateW[?, S, ?], S] = castW(scalaLifecycleStateWInstance)
}
...
trait StateAccessorImplicits1 extends StateAccessorImplicits2 {
  ...
  private def newScalaLifecycleStateRWCB[S]: ReadWritePure[Lifecycle.StateRW[?, S, ?], S] = {
    type I = Lifecycle.StateRW[?, S, ?]
    ReadWrite[I, DS, DS, DA, S](i => DS.pure(i.state), identityFn)
  }
  private lazy val scalaLifecycleStateRWCBInstance = newScalaLifecycleStateRWCB[Any]
  implicit def scalaLifecycleStateRWCB[S]: ReadWritePure[Lifecycle.StateRW[?, S, ?], S] = castRW(scalaLifecycleStateRWCBInstance)
}
...
trait StateAccessorImplicits extends StateAccessorImplicits1 {
  ...
  private def newScalaLifecycleStateRW[S]: ReadImpureWritePure[Lifecycle.StateRW[?, S, ?], S] = {
    type I = Lifecycle.StateRW[?, S, ?]
    ReadWrite[I, Id, DS, DA, S](_.state, identityFn)
  }
  private lazy val scalaLifecycleStateRWInstance = newScalaLifecycleStateRW[Any]
  implicit def scalaLifecycleStateRW[S]: ReadImpureWritePure[Lifecycle.StateRW[?, S, ?], S] = castRW(scalaLifecycleStateRWInstance)
}
