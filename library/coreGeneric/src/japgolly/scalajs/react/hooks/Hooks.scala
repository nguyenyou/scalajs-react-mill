...
object Hooks {
  ...
  object UseReducer {
    ...
    implicit def reusability[S, A](implicit S: Reusability[S]): Reusability[UseReducer[S, A]] = {
      val r = implicitly[Reusability[Reusable[facade.React.UseReducerDispatch[?]]]]
      Reusability((x, y) => S.test(x.value, y.value) && r.test(x.originalDispatch, y.originalDispatch))
    }
  }

  final case class UseReducer[S, A](raw: facade.React.UseReducer[S, A], originalDispatch: Reusable[facade.React.UseReducerDispatch[?]]) {
  ...
  object UseRef {
    ...
    def unsafeCreateToJsComponentWithMountedFacade[P <: js.Object, S <: js.Object, F <: js.Object](): Ref.ToJsComponent[P, S, JsComponent.RawMounted[P, S] & F] =
      unsafeCreateSimple[JsComponent.RawMounted[P, S] & F]().map(JsComponent.mounted[P, S](_).addFacade[F])
