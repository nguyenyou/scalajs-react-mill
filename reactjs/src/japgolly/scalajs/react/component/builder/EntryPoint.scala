...
object EntryPoint {
  ...
  inline def apply[Props](inline displayName: String): Step1[Props] =
    new Step1[Props](
      ScalaJsReactConfig.Instance.modifyComponentName(displayName)
    )

  /** Partially builds a component that always displays the same content, never
    * needs to be redrawn, never needs vdom diffing. The builder is returned and
    * can be customised futher before finally being built.
    */
  inline def static(inline displayName: String)(
      content: VdomNode
  ): LastStep[Unit, Children.None, Unit, Unit, UpdateSnapshot.None] =
    apply[Unit](displayName).noBackend
