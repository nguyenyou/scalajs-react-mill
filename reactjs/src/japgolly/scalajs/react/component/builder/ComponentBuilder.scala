...
object ComponentBuilder {
  ...
  inline given defaultToNoBackend[X, P, S](using
      inline ev: X => Step2[P, S]
  ): Conversion[X, Step3[P, S, Unit]] =
    x =>
      inline x match {
  ...
  /** You're on step 1/4 on the way to building a component.
    *
    * If your component is to be stateful, here you must specify it's initial
    * state.
    *
    * If your component is to be stateless, you can skip this step or explicitly
    * use `.stateless`.
    */
  final class Step1[P](private val name: String) extends AnyVal {
    ...
    /** getDerivedStateFromProps is invoked right before calling the render
      ...
      * in props over time. For example, it might be handy for implementing a
      ...
      * This is in contrast to componentWillReceiveProps, which only fires when
      * the parent causes a re-render and not as a result of a local setState.
      */
    @deprecated(
      "Use getDerivedStateFromPropsAndState instead. This doesn't just get called when props change, it gets called when state changes too; meaning it gets reset every time you call setState (!)",
      "1.7.1"
    )
    def getDerivedStateFromProps[S](f: P => S): Step2[P, S] =
      new Step2(name, InitState.DerivedFromProps(f))

    /** This is called twice when a component is first rendered. Once with state
      * set to `None` and then again by React with state set to `Some`.
      *
      * getDerivedStateFromProps is invoked right before calling the render
      * method, both on the initial mount and on subsequent updates.
      *
      ...
      * the parent causes a re-render and not as a result of a local setState.
      */
    def getDerivedStateFromPropsAndState[S](
        f: (P, Option[S]) => S
    ): Step2[P, S] =
    ...
    def initialStateCallback[G[_], S](cb: G[S])(implicit
        G: Sync[G]
    ): Step2[P, S] =
      initialState(G.runSync(cb))

    def initialStateCallbackFromProps[G[_], S](f: P => G[S])(implicit
        G: Sync[G]
    ): Step2[P, S] =
  ...
  /** You're on step 2/4 on the way to building a component.
    *
    * Here you specify whether your component has a "backend" or not. A backend
    * like a class that is created when your component mounts and remains until
    * it is unmounted.
    *
    * If you don't need a backend, you can skip this step or explicitly use
    * `.noBackend`.
    *
    * Making common cases convenient, you can even use `.renderBackend` or
    * `.renderBackendWithChildren` to take care of both this and the following
    * step automatically, using macros to save you typing boring boilerplate. If
    * you have an unhealthy fear of macros you can ignore then and do it all
    * manually too; the macros don't have any special privileges.
    */
  final class Step2[P, S](name: String, initState: InitState[P, S]) {
    ...
    /** Shortcut for:
      ...
      * }}}
      */
    inline def renderBackend[B]
        : LastStep[P, Children.None, S, B, UpdateSnapshot.None] =
    ...
    /** Shortcut for:
      ...
      * }}}
      */
    inline def renderBackendWithChildren[B]
        : LastStep[P, Children.Varargs, S, B, UpdateSnapshot.None] =
      backend(
        ComponentBuilderMacros.newBackendFn[P, S, B]
      ).renderBackendWithChildren
  }
  ...
  /** You're on step 3/4 on the way to building a component.
    *
    * Here you specify your component's render function. For convenience there
    * are a plethora of methods you can call that all do the same thing but
    * accept the argument in different shapes; the suffixes in method names
    * refer to the argument type(s). This means you choose the method that
    * provides the types that you need, rather than having to manually specify
    * the types for all arguments including stuff you don't need.
    *
    * If you're using a backend, then it's highly recommended that you put your
    * render function in the backend. When you do that, make sure it's called
    * `.render` and then here in the builder, use the `.renderBackend` or
    * `.renderBackendWithChildren` methods which will use a macro to inspect
    * your backend's render method and provide everything it needs
    * automatically.
    */
  final class Step3[P, S, B](
      name: String,
      initState: InitState[P, S],
      backendFn: NewBackendFn[P, S, B]
  ) {

    type $ = RenderScope[P, S, B]

    def renderWith[C <: Children](
        r: RenderFn[P, S, B]
    ): LastStep[P, C, S, B, UpdateSnapshot.None] = {
      var lc = Lifecycle.empty[P, S, B]
      initState match {
        case InitState.DerivedFromProps(f) =>
          lc = lc.copy(getDerivedStateFromProps = Some((p, _) => Some(f(p))))
        case InitState.DerivedFromPropsAndState(f) =>
          lc = lc.copy(getDerivedStateFromProps =
            Some((p, s) => Some(f(p, Some(s))))
          )
        case _ => ()
      }
      new LastStep[P, C, S, B, UpdateSnapshot.None](
        name,
        initState,
        backendFn,
        r,
    ...
    def renderPCS(
        r: ($, P, PropsChildren, S) => VdomNode
    ): LastStep[P, Children.Varargs, S, B, UpdateSnapshot.None] =
      renderWith($ => r($, $.props, $.propsChildren, $.state))

    def renderPC(
        r: ($, P, PropsChildren) => VdomNode
    ): LastStep[P, Children.Varargs, S, B, UpdateSnapshot.None] =
      renderWith($ => r($, $.props, $.propsChildren))

    def renderCS(
        r: ($, PropsChildren, S) => VdomNode
    ): LastStep[P, Children.Varargs, S, B, UpdateSnapshot.None] =
      renderWith($ => r($, $.propsChildren, $.state))

    def renderC(
        r: ($, PropsChildren) => VdomNode
    ): LastStep[P, Children.Varargs, S, B, UpdateSnapshot.None] =
    ...
    /** Use a method named `render` in the backend, automatically populating its
      * arguments with props and state where needed.
      */
    inline def renderBackend
        : LastStep[P, Children.None, S, B, UpdateSnapshot.None] =
      renderWith(ComponentBuilderMacros.renderBackendFn)

    /** Use a method named `render` in the backend, automatically populating its
      * arguments with props, state, and propsChildren where needed.
      */
    inline def renderBackendWithChildren
        : LastStep[P, Children.Varargs, S, B, UpdateSnapshot.None] =
  ...
  /** You're on the final step on the way to building a component.
    ...
    *
    * When you're done, simply call `.build` to create and return your
    * `ScalaComponent`.
    */
  final class LastStep[P, C <: Children, S, B, US <: UpdateSnapshot](
      ...
      private[builder] val lifecycle: Lifecycle[P, S, B, UpdateSnapshot.Value[
        US
      ]]
  ) {
    ...
    private def setLC[US2 <: UpdateSnapshot](
        lc: Lifecycle[P, S, B, UpdateSnapshot.Value[US2]]
    ): LastStep[P, C, S, B, US2] =
      new LastStep(name, initState, backendFn, renderFn, lc)

    private def lcAppend[I, O](lens: Lens[Lifecycle_, Option[I => O]])(
        g: I => O
    )(implicit s: Semigroup[O]): This =
      copy(lifecycle = lifecycle.append(lens)(g)(s))

    private def lcAppendDispatch[G[_], I](
        lens: Lens[Lifecycle_, Option[I => DS[Unit]]]
    )(g: I => G[Unit])(implicit G: Dispatch[G]): This =
      copy(lifecycle =
        lifecycle.append(lens)(DS.transDispatchFn1(g))(DS.semigroupSyncUnit)
      )

    inline def configure[US2 <: UpdateSnapshot](
        f: Config[P, C, S, B, US, US2]
    ): LastStep[P, C, S, B, US2] =
      f(this)

    inline def configureWhen(inline cond: Boolean)(
        inline f: Config[P, C, S, B, US, US]
    ): LastStep[P, C, S, B, US] =
      if (cond) configure(f) else this

    inline def configureUnless(inline cond: Boolean)(
        inline f: Config[P, C, S, B, US, US]
    ): LastStep[P, C, S, B, US] =
      if (cond) this else configure(f)

    /** Error boundaries are React components that catch errors anywhere in
      * their child component tree, log those errors, and display a fallback UI
      * instead of the component tree that crashed. Error boundaries catch
      * errors during rendering, in lifecycle methods, and in constructors of
      * the whole tree below them.
      *
      * Note: "CHILD COMPONENT TREE". Components cannot catch errors in
      * themselves, only their children.
      */
    def componentDidCatch[G[_]](f: ComponentDidCatch[P, S, B] => G[Unit])(
    ...
    /** Invoked once, only on the client (not on the server), immediately after
      ...
      * `ReactDOM.findDOMNode(this)`. The `componentDidMount()` method of child
      ...
      * using `setTimeout` or `setInterval`, or send AJAX requests, perform
      * those operations in this method.
      */
    def componentDidMount[G[_]](f: ComponentDidMount[P, S, B] => G[Unit])(
        implicit G: Dispatch[G]
    ): This =
      lcAppendDispatch(LifecycleF.componentDidMount)(f)

    /** Invoked immediately after the component's updates are flushed to the
      * DOM. This method is not called for the initial render.
      *
      * Use this as an opportunity to operate on the DOM when the component has
      * been updated.
      */
    def componentDidUpdate[G[_]](
        f: ComponentDidUpdate[P, S, B, SnapshotValue] => G[Unit]
    )(implicit
        G: Dispatch[G]
    ): LastStep[P, C, S, B, UpdateSnapshot.Some[SnapshotValue]] =
      setLC[UpdateSnapshot.Some[SnapshotValue]](
        lcAppendDispatch(LifecycleF.componentDidUpdate)(f).lifecycle
      )
    ...
    def componentWillMount[G[_]](f: ComponentWillMount[P, S, B] => G[Unit])(
        implicit G: Dispatch[G]
    ): This =
    ...
    /** Invoked when a component is receiving new props. This method is not
      ...
      *
      * Note: There is no analogous method `componentWillReceiveState`. An
      * incoming prop transition may cause a state change, but the opposite is
      * not true. If you need to perform operations in response to a state
      * change, use `componentWillUpdate`.
      */
    @deprecated(
    ...
    def componentWillReceiveProps[G[_]](
        f: ComponentWillReceiveProps[P, S, B] => G[Unit]
    )(implicit G: Dispatch[G]): This =
    ...
    /** Invoked immediately before a component is unmounted from the DOM.
      ...
      * `componentDidMount`.
      */
    def componentWillUnmount[G[_]](f: ComponentWillUnmount[P, S, B] => G[Unit])(
        implicit G: Dispatch[G]
    ): This =
    ...
    /** Invoked immediately before rendering when new props or state are being
      ...
      * occurs.
      *
      * Note: You *cannot* use `this.setState()` in this method. If you need to
    ...
    def componentWillUpdate[G[_]](f: ComponentWillUpdate[P, S, B] => G[Unit])(
        implicit G: Dispatch[G]
    ): This =
    ...
    /** getDerivedStateFromProps is invoked right before calling the render
      ...
      * return Some to update the state, or None to update nothing.
      *
      * This method exists for rare use cases where the state depends on changes
      * in props over time. For example, it might be handy for implementing a
      * Transition component that compares its previous and next children to
    ...
    /** getSnapshotBeforeUpdate() is invoked right before the most recently
      ...
      * it is potentially changed. Any value returned by this lifecycle will be
      * passed as a parameter to componentDidUpdate().
      *
      * This use case is not common, but it may occur in UIs like a chat thread
      * that need to handle scroll position in a special way.
      */
    @nowarn("cat=unused")
    def getSnapshotBeforeUpdate[G[_], SS](
        f: GetSnapshotBeforeUpdate[P, S, B] => G[SS]
    )(implicit
        ev: UpdateSnapshot.SafetyProof[US],
        G: Sync[G]
    ): LastStep[P, C, S, B, UpdateSnapshot.Some[SS]] =
      setLC[UpdateSnapshot.Some[SS]](
        lifecycle.resetSnapshot(None, Some(DS.transSyncFn1(f)))
      )
    ...
    /** getSnapshotBeforeUpdate() is invoked right before the most recently
      ...
      * that need to handle scroll position in a special way.
      */
    def getSnapshotBeforeUpdatePure[A](
        f: GetSnapshotBeforeUpdate[P, S, B] => A
    )(implicit
        ev: UpdateSnapshot.SafetyProof[US]
    ): LastStep[P, C, S, B, UpdateSnapshot.Some[A]] =
    ...
    /** Invoked before rendering when new props or state are being received.
      ...
      * always treat `state` as immutable and to read only from `props` and
      ...
      */
    def shouldComponentUpdate[G[_]](
        f: ShouldComponentUpdate[P, S, B] => G[Boolean]
    )(implicit G: Sync[G]): This =
      lcAppend(LifecycleF.shouldComponentUpdate)(DS.transSyncFn1(f))(
        DS.semigroupSyncOr
      )
    ...
    /** Invoked before rendering when new props or state are being received.
      ...
      * components, use `shouldComponentUpdate` to speed up your app.
      */
    def shouldComponentUpdatePure(
        f: ShouldComponentUpdate[P, S, B] => Boolean
    ): This =
      shouldComponentUpdate($ => DS.delay(f($)))

    def componentDidCatchConst[G[_]](f: G[Unit])(implicit
        G: Dispatch[G]
    ): This = { val x = DS.transDispatch(f); componentDidCatch(_ => x) }
    def componentDidMountConst[G[_]](f: G[Unit])(implicit
        G: Dispatch[G]
    ): This = { val x = DS.transDispatch(f); componentDidMount(_ => x) }
    def componentDidUpdateConst[G[_]](f: G[Unit])(implicit G: Dispatch[G]) = {
      val x = DS.transDispatch(f); componentDidUpdate(_ => x)
    }
    def componentWillUnmountConst[G[_]](f: G[Unit])(implicit
        G: Dispatch[G]
    ): This = { val x = DS.transDispatch(f); componentWillUnmount(_ => x) }
    def shouldComponentUpdateConst[G[_]](f: G[Boolean])(implicit
        G: Sync[G]
    ): This = { val x = DS.transSync(f); shouldComponentUpdate(_ => x) }
    def shouldComponentUpdateConst(b: Boolean): This =
      shouldComponentUpdateConst(DS.pure(b))

    @deprecated(
      "Use either .initialState* on the component builder, or .componentDidMount. See https://reactjs.org/docs/react-component.html#unsafe_componentwillmount / https://reactjs.org/blog/2018/03/27/update-on-async-rendering.html",
      "scalajs-react 1.7.0 / React 16.9.0"
    )
    def componentWillMountConst[G[_]](cb: G[Unit])(implicit
        G: Dispatch[G]
    ): This = componentWillMount(_ => cb)

    @deprecated(
      "See https://reactjs.org/docs/react-component.html#unsafe_componentwillreceiveprops / https://reactjs.org/blog/2018/03/27/update-on-async-rendering.html",
      "scalajs-react 1.7.0 / React 16.9.0"
    )
    def componentWillReceivePropsConst[G[_]](cb: G[Unit])(implicit
        G: Dispatch[G]
    ): This = componentWillReceiveProps(_ => cb)

    @deprecated(
      "Use .componentDidUpdate or .getSnapshotBeforeUpdate. See https://reactjs.org/docs/react-component.html#unsafe_componentwillupdate / https://reactjs.org/blog/2018/03/27/update-on-async-rendering.html",
      "scalajs-react 1.7.0 / React 16.9.0"
    )
    def componentWillUpdateConst[G[_]](cb: G[Unit])(implicit
        G: Dispatch[G]
    ): This = componentWillUpdate(_ => cb)
  ...
  def fromReactComponentClass[P, C <: Children, S, B](
      rc: facade.React.ComponentClass[Box[P], Box[S]]
  )(implicit
      ctorType: CtorType.Summoner[Box[P], C]
  ): Scala.Component[P, S, B, ctorType.CT] =
