...
object ComponentBuilderMacros {
  ...
  private def newBackendFnImpl[P: Type, S: Type, B: Type](using
      Quotes
  ): Expr[NewBackendFn[P, S, B]] = {
    ...
    def lambdaBody(input: Expr[Input]): Expr[B] = {
      MacroUtils.NewInstance.of[B](
        findTermArg = Some { (valDef, fail) =>
          ...
          if isBackendScope(t) then input.asTerm
          else fail()
        }
      )
    }
  ...
  def renderBackendFnImpl[P: Type, S: Type, B: Type](allowChildren: Boolean)(
      using Quotes
  ): Expr[RenderFn[P, S, B]] =
  ...
  private def renderParams[P: Type, S: Type, B: Type](using
      Quotes
  ): List[RenderParam[P, S, B, ?]] =
...
//      Nil)
  ...
  private final class RenderParam[P, S, B, T: Type](
      val select: Quotes ?=> Expr[RenderScope[P, S, B]] => Expr[T]
  )(_names: String*) {
  ...
  private def renderBackendFnImpl[P: Type, S: Type, B: Type](
      allowChildren: Boolean,
      rps: List[RenderParam[P, S, B, _]]
      ...
      inline def byExactTypeAlias(t: TypeRepr) = {
        if (debug) {
          ...
          for (p <- rps) {
            ...
            val b = t.typeSymbol
            debugPrn(
              s"    - ${p.names.mkString("/")}: T($b) == RP($a) ? ${a == b} .... ${p.spec =:= t} ${p.spec.dealias =:= t} ${p.spec =:= t.dealias}"
            )
          }
        }
        ...
        (
          tryPropsChildren(pt) orElse
            ...
            fail {
              ...
              // for users too.
              if paramType
                  .startsWith("scala.") && paramType.indexOf('.', 6) == -1
              then paramType = paramType.drop(6)
    ...
    val renderMethod: Symbol = {
      ...
      candidates match {
        ...
        case m :: Nil => m
        case _ =>
          fail(s"${Type.show[B]} contains multiple public render methods.")
      }
    }
    ...
    val params: Option[List[Symbol]] = {
      ...
      paramSymss match {
        ...
        case _ =>
          fail(
            s"${Type.show[B]}.render mustn't have more than one set of parameters."
          )
      }
    }
