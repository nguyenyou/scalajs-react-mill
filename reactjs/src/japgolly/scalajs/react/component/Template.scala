...
object Template {
  ...
  abstract class MountedWithRoot[F[_], A[_], P0, S0](implicit
      ft: UnsafeSync[F],
      at: Async[A]
  ) extends Generic.MountedWithRoot[F, A, P0, S0, P0, S0] {

    type Mapped[F1[_], A1[_], P1, S1] <: Generic.MountedWithRoot[
      F1,
      A1,
      P1,
      S1,
    ...
    protected def mapped[F1[_], A1[_], P1, S1](
        mp: P0 => P1,
        ls: Lens[S0, S1]
  ...
  abstract class MountedMapped[F[_], A[_], P2, S2, P1, S1, P0, S0](
  ...
  )(mp: P1 => P2, ls: Lens[S1, S2])(implicit ft: UnsafeSync[F], at: Async[A])
      extends Generic.MountedWithRoot[F, A, P2, S2, P0, S0] {

    type Mapped[F3[_], A3[_], P3, S3] <: Generic.MountedWithRoot[
      F3,
      A3,
      P3,
      S3,
    ...
    protected def mapped[F3[_], A3[_], P3, S3](
        mp: P1 => P3,
        ls: Lens[S1, S3]
    )(implicit ft: UnsafeSync[F3], at: Async[A3]): Mapped[F3, A3, P3, S3]
    ...
    override def setState[G[_]](s: State, callback: => G[Unit])(implicit
        G: Dispatch[G]
    ) =
      ft.transSync(from.modState(ls set s, callback))

    override def modState[G[_]](f: State => State, callback: => G[Unit])(
        implicit G: Dispatch[G]
    ) =
      ft.transSync(from.modState(ls mod f, callback))

    override def modState[G[_]](
        f: (State, Props) => State,
        callback: => G[Unit]
    )(implicit G: Dispatch[G]) =
      ft.transSync(
        from.modState((s1, p1) => ls.set(f(ls.get(s1), mp(p1)))(s1), callback)
      )

    override def setStateOption[G[_]](o: Option[State], callback: => G[Unit])(
        implicit G: Dispatch[G]
    ) =
      o.fold(ft.transSync(from.setStateOption(None, callback)))(
        setState(_, callback)
      )
    ...
    override def modStateOption[G[_]](
        f: (State, Props) => Option[State],
        callback: => G[Unit]
    )(implicit G: Dispatch[G]) =
      ft.transSync(
        from.modStateOption(
          (s1, p1) => f(ls.get(s1), mp(p1)).map(ls.set(_)(s1)),
          callback
        )
      )
