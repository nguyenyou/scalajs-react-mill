...
object JsFn
    extends JsBaseComponentTemplate[facade.React.StatelessFunctionalComponent] {

  type Component[P <: js.Object, CT[-p, +u] <: CtorType[p, u]] =
    ComponentRoot[P, CT, Unmounted[P]]
  type Unmounted[P <: js.Object] = UnmountedRoot[P]
  type Mounted = Unit

  def apply[P <: js.Object, C <: Children](raw: js.Any)(implicit
  ...
  def force[P <: js.Object, C <: Children](
      raw: Any
  )(implicit s: CtorType.Summoner[P, C]): Component[P, s.CT] = {
    val rc = raw.asInstanceOf[facade.React.StatelessFunctionalComponent[P]]
    componentRoot[P, s.CT, Unmounted[P]](
      rc,
      s.pf.rmap(s.summon(rc))(unmountedRoot)
    )(s.pf)
  }

  def fromJsFn[P <: js.Object, C <: Children](
      render: js.Function1[
        P & facade.PropsWithChildren,
        facade.React.Element
      ]
  )(implicit s: CtorType.Summoner[P, C]): Component[P, s.CT] =
  ...
  object fromScala {
    ...
    def generic[P <: js.Object, C <: Children](
        render: P & facade.PropsWithChildren => VdomElement
    )(implicit s: CtorType.Summoner[P, C]): Component[P, s.CT] =
      fromJsFn[P, C]((p: P & facade.PropsWithChildren) =>
        render(p).rawElement
      )(s)

    def const(render: VdomElement)(implicit
        s: CtorType.Summoner[UnusedObject, Children.None]
    ): Component[UnusedObject, s.CT] =
      generic[UnusedObject, Children.None](_ => render)(s)

    def delay(render: => VdomElement)(implicit
        s: CtorType.Summoner[UnusedObject, Children.None]
    ): Component[UnusedObject, s.CT] =
      generic[UnusedObject, Children.None](_ => render)(s)

    @deprecated("Use .delay", "2.0.0")
    def byName(render: => VdomElement)(implicit
        s: CtorType.Summoner[UnusedObject, Children.None]
    ): Component[UnusedObject, s.CT] =
      delay(render)(s)

    def apply[P <: js.Object](render: P => VdomElement)(implicit
        s: CtorType.Summoner[P, Children.None]
    ): Component[P, s.CT] =
      generic[P, Children.None](render)(s)

    def withChildren[P <: js.Object](render: (P, PropsChildren) => VdomElement)(
        implicit s: CtorType.Summoner[P, Children.Varargs]
    ): Component[P, s.CT] =
      generic[P, Children.Varargs](p => render(p, PropsChildren(p.children)))(s)

    def justChildren(render: PropsChildren => VdomElement)(implicit
        s: CtorType.Summoner[UnusedObject, Children.Varargs]
    ): Component[UnusedObject, s.CT] =
      generic[UnusedObject, Children.Varargs](p =>
        render(PropsChildren(p.children))
      )(s)
  }
  ...
  override protected def rawComponentDisplayName[A <: js.Object](
      r: facade.React.StatelessFunctionalComponent[A]
  ) =
    readDisplayName(r)
  ...
  @implicitNotFound(
    "Don't know how to convert ${P} into a JS object. Try adding .cmapCtorProps to your component."
  )
  ...
  object ToRawCtor {
    type Aux[
        ...
        Scala,
        C <: Children,
        CTS <: CtorType.Summoner[J, C]
    ] =
      ToRawCtor[Scala, C] { type JS = J; val cts: CTS }

    implicit def id[J <: js.Object, C <: Children](implicit
        s: CtorType.Summoner[J, C]
    ): Aux[J, J, C, s.type] =
      new ToRawCtor[J, C] {
        override type JS = J
    ...
    implicit def singleton[P, C <: Children](implicit
        p: Singleton[P],
        s: CtorType.Summoner[UnusedObject, C]
    ): Aux[UnusedObject, P, C, s.type] =
      new ToRawCtor[P, C] {
  ...
  @implicitNotFound(
    "Don't know how to convert unmounted value into a React.Element. Try adding .mapUnmounted to your component."
  )
  final case class ToRawReactElement[-A](run: A => facade.React.Element)
      extends AnyVal
  object ToRawReactElement {
    implicit def id: ToRawReactElement[facade.React.Element] = apply(identityFn)
    implicit def fromVdom: ToRawReactElement[vdom.VdomElement] = apply(
      _.rawElement
    )
    implicit def fromUnmounted[P, M]
        : ToRawReactElement[Generic.UnmountedSimple[P, M]] = apply(
      _.vdomElement.rawElement
    )
  }
  ...
  sealed trait UnmountedSimple[P, M] extends Generic.UnmountedSimple[P, M] {
    ...
    override final def renderIntoDOM(
        container: facade.ReactDOM.Container
    ): this.Mounted =
      postRender(facade.ReactDOM.render(raw, container))

    override final def renderIntoDOM[G[_]](
        container: facade.ReactDOM.Container,
        callback: => G[Unit]
    )(implicit G: Dispatch[G]): this.Mounted =
      postRender(facade.ReactDOM.render(raw, container, G.dispatchFn(callback)))

    private def postRender(
        result: facade.React.ComponentUntyped
    ): this.Mounted = {
      // Protect against future React change.
      if (developmentMode)
        assert(
          result eq null,
          "Expected rendered functional component to return null; not " + result
        )
      mountRaw(result)
    }
  }
  ...
  sealed trait UnmountedWithRoot[P1, M1, P0 <: js.Object]
      ...
      with Generic.UnmountedWithRoot[P1, M1, P0, Mounted] {
  ...
  private def mappedU[P2, M2, P1, M1, P0 <: js.Object](
      from: UnmountedWithRoot[P1, M1, P0]
