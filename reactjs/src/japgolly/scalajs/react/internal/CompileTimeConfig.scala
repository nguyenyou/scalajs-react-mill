...
object CompileTimeConfig {
  ...
  transparent inline def getTrimLowerCaseNonBlank(
      inline key: String
  ): Option[String] =
    TransparentInlineUtils.trimLowerCaseNonBlank(get(key))

  transparent inline def getOrUseModule[A](
      inline key: String,
      inline fallback: A
  ): A =
  ...
  object quoted {
    ...
    def getOrUseModule[A: Type](key: Expr[String], fallback: Expr[A])(using
        Quotes
    ): Expr[A] = {
      import quotes.reflect.*
    ...
    def loadModuleOrError[A: Type](
        fqcnExpr: Expr[String]
    )(using Quotes): Expr[A] = {
    ...
    def loadModule[A: Type](
        fqcn: String
    )(using Quotes): Either[String, Expr[A]] = {
      import quotes.reflect.*
      ...
      if tpe <:< TypeRepr.of[A] then Right(ref.asInlineExprOf[A])
      else if !tpe.exists then Left("doesn't exist")
      else Left("isn't an instance of " + Type.show[A])
    }
