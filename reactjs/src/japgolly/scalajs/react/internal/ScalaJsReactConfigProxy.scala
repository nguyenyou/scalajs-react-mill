...
object ScalaJsReactConfigProxy {
  ...
  transparent inline def automaticComponentName(name: String) = ${
    _automaticComponentName('name)
  }
  transparent inline def modifyComponentName(name: String) = ${
    _modifyComponentName('name)
  }
  ...
  private def call[A](
    ...
    def resolve(moduleExpr: Expr[ScalaJsReactConfig]): Expr[A] = {
      ...
      val methodSym =
        candidateSyms.iterator
          .filter(_.signature.paramSigs == paramSigs)
          .nextOption()
          .getOrElse {
            val sigs = candidateSyms
              .map(_.signature.paramSigs.mkString(", "))
              .sorted
              .map(p => s"  - $methodName($p)")
            fail(
              s"$name doesn't have the right signature. Found:\n" + sigs
                .mkString("\n")
            )
          }
      ...
      if (moduleExpr ne defaultModule) {
        ...
        if (
          owner.flags.is(
            Flags.Trait
          ) && owner.fullName == "japgolly.scalajs.react.ScalaJsReactConfig$.Defaults"
        )
          forceDefault = true
      }
    ...
    resolve(
      CompileTimeConfig.quoted
        .getModule[ScalaJsReactConfig](Expr.inlineConst(KeyConfigClass))
