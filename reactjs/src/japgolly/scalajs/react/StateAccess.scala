...
/** Base class for something that has read/write state access (under the same
  * effect type).
  *
  * Passing this around (top-level) is fine but do not use it in a
  * generic/library/helper method. In intermediary positions, use
  * [[StateAccessor]] instead.
  *
  * @tparam F
...
object StateAccess {
  ...
  trait SetState[F[_], A[_], S] extends Any with Base[F, A] {
    ...
    def setState[G[_]](newState: S, callback: => G[Unit])(implicit
        G: Dispatch[G]
    ): F[Unit] =
    ...
    def setStateOption[G[_]](newState: Option[S], callback: => G[Unit])(implicit
        G: Dispatch[G]
    ): F[Unit]
  ...
  trait ModState[F[_], A[_], S] extends Any with Base[F, A] {
    ...
    /** @param callback Executed after state is changed. */
    def modState[G[_]](mod: S => S, callback: => G[Unit])(implicit
        G: Dispatch[G]
    ): F[Unit] =
    ...
    /** @param callback Executed regardless of whether state is changed. */
    def modStateOption[G[_]](mod: S => Option[S], callback: => G[Unit])(implicit
        G: Dispatch[G]
    ): F[Unit]
  ...
  trait ModStateWithProps[F[_], A[_], P, S] extends Any with Base[F, A] {
    ...
    /** @param callback Executed after state is changed. */
    def modState[G[_]](mod: (S, P) => S, callback: => G[Unit])(implicit
        G: Dispatch[G]
    ): F[Unit] =
    ...
    /** @param callback Executed regardless of whether state is changed. */
    def modStateOption[G[_]](mod: (S, P) => Option[S], callback: => G[Unit])(
  ...
  trait WriteWithProps[F[_], A[_], P, S]
      extends Any
      with Write[F, A, S]
      with ModStateWithProps[F, A, P, S] {
  ...
  def apply[F[_], A[_], S](stateFn: => F[S])(
    ...
    new StateAccess[F, A, S] {
      ...
      override def setStateOption[G[_]](
