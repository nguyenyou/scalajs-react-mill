...
object CallbackOption {
  ...
  inline def optionCallback[A](
      inline oc: Option[CallbackTo[A]]
  ): CallbackOption[A] =
    CallbackTo.sequenceOption(oc).asCBO

  def maybeCallback[O[_], A](oa: => O[CallbackTo[A]])(implicit
      O: OptionLike[O]
  ): CallbackOption[A] =
  ...
  def liftOptionLike[O[_], A](oa: => O[A])(implicit
      O: OptionLike[O]
  ): CallbackOption[A] =
    maybe(oa)
  ...
  def liftOptionLikeCallback[O[_], A](oa: => O[CallbackTo[A]])(implicit
      O: OptionLike[O]
  ): CallbackOption[A] =
    maybeCallback(oa)
  ...
  inline def matchPF[A, B](inline a: A)(
  ...
  def tailrec[A, B](
      a: A
  )(f: A => CallbackOption[Either[A, B]]): CallbackOption[B] =
    option {
      @tailrec
      def go(a: A): Option[B] =
  ...
  def sequence[T[X] <: Iterable[X], A](tca: => T[CallbackOption[A]])(using
      cbf: BuildFrom[T[CallbackOption[A]], A, T[A]]
  ): CallbackOption[T[A]] =
...
/** Callback that can short-circuit along the way when conditions you specify,
  * aren't met.
  *
  * Especially useful for event handlers such as key handlers, drag-and-drop
  * handlers, etc, where you check a condition, perform an effect, check another
  * condition, perform another effect, etc.
  *
  * This is meant to be lightweight, and be immediately useful without the
  * typical pain of imports, implicit conversions and extension methods then
  * normally accompany monad transforms in Scala.
  *
  * For a more generic (i.e. beyond Option) or comprehensive monad transformer
  * use Cats or similar.
  */
final class CallbackOption[+A](
    val underlyingRepr: CallbackOption.UnderlyingRepr[A]
) extends AnyVal { self =>
  ...
  /** Alias for `orElse`.
    */
  inline def |[AA >: A](
      inline tryNext: CallbackOption[AA]
  ): CallbackOption[AA] =
    orElse(tryNext)
