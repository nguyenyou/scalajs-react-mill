...
object CallbackTo {
  ...
  final class LiftTraverseDsl[A, B](private val f: A => CallbackTo[B])
      extends AnyVal {
    ...
    def std[T[X] <: Iterable[X]](implicit
        cbf: BuildFrom[T[A], B, T[B]]
    ): CallbackTo[T[A] => T[B]] =
      CallbackTo { ta =>
  ...
  /** Traverse stdlib T over CallbackTo. Distribute CallbackTo over stdlib T.
    */
  def traverse[T[X] <: Iterable[X], A, B](ta: => T[A])(f: A => CallbackTo[B])(
      implicit cbf: BuildFrom[T[A], B, T[B]]
  ): CallbackTo[T[B]] =
    liftTraverse(f).std[T](cbf).map(_(ta))

  /** Sequence stdlib T over CallbackTo. Co-sequence CallbackTo over stdlib T.
    */
  inline def sequence[T[X] <: Iterable[X], A](inline tca: T[CallbackTo[A]])(
      implicit cbf: BuildFrom[T[CallbackTo[A]], A, T[A]]
  ): CallbackTo[T[A]] =
    traverse(tca)(identityFn)(cbf)

  /** Traverse Option over CallbackTo. Distribute CallbackTo over Option.
    */
  def traverseOption[A, B](oa: => Option[A])(
      f: A => CallbackTo[B]
  ): CallbackTo[Option[B]] =
  ...
  /** Sequence Option over CallbackTo. Co-sequence CallbackTo over Option.
    */
  inline def sequenceOption[A](
      inline oca: Option[CallbackTo[A]]
  ): CallbackTo[Option[A]] =
    traverseOption(oca)(identityFn)

  /** Wraps a [[Future]] so that it is repeatable, and so that its inner
    * callback is run when the future completes.
    *
    * WARNING: Futures are scheduled to run as soon as they're created. Ensure
    * that the argument you provide creates a new [[Future]]; don't reference an
    * existing one.
    */
  inline def future[A](f: Future[CallbackTo[A]])(implicit
      ec: ExecutionContext
  ): CallbackTo[Future[A]] =
  ...
  /** Displays a dialog with an optional message prompting the user to input
    * some text.
    *
    * @return
  ...
  /** Displays a dialog with an optional message prompting the user to input
    ...
    *
    * @param message
    *   a string of text to display to the user. This parameter is optional and
  ...
  /** Displays a dialog with an optional message prompting the user to input
    ...
    *   It is an optional parameter. Note that in Internet Explorer 7 and 8, if
    *   you do not provide this parameter, the string "undefined" is the default
    *   value.
    * @return
    *   Some string comprising the text entered by the user, or None.
    */
  ...
  /** Displays a modal dialog with a message and two buttons, OK and Cancel.
    ...
    * @return
    *   A boolean value indicating whether OK or Cancel was selected (true means
    *   OK).
    */
  def confirm(message: String): CallbackTo[Boolean] =
    apply(window.confirm(message))

  def retryUntilRight[L, R](
      attempt: CallbackTo[Either[L, R]]
  )(onLeft: L => Callback): CallbackTo[R] =
    tailrec[Unit, R](())(_ =>
      attempt.flatMap {
  ...
  /** Serves as a temporary placeholder for a callback until you supply a real
    * implementation.
    *
    * Unlike `???` this doesn't crash, it just prints a warning to the console.
    *
    * Also it's not really deprecated; that's just so you get a compiler warning
    * as a reminder.
    */
  ...
  /** Prevents `scalac` discarding the result of a map function when the final
    * result is `Callback`.
    *
  ...
  extension (self: CallbackTo[Boolean]) {
    ...
    /** Creates a new callback that returns `true` when both this and the given
      * callback return `true`.
      */
    def &&(b: CallbackTo[Boolean]): CallbackTo[Boolean] =
      self.map(_ && b.runNow())

    /** Creates a new callback that returns `true` when either this or the given
      * callback return `true`.
      */
    def ||(b: CallbackTo[Boolean]): CallbackTo[Boolean] =
      self.map(_ || b.runNow())
    ...
    /** Returns a [[CallbackOption]] that requires the boolean value therein to
      * be true.
      */
...
/** A function to be executed later, usually by scalajs-react in response to
  * some kind of event.
  *
  * The purpose of this class is to lift effects into the type system, and use
  * the compiler to ensure safety around callbacks (without depending on an
  * external library like Cats).
  *
  * `() => Unit` is replaced by `Callback`. Similarly, `ReactEvent => Unit` is
  * replaced by `ReactEvent => Callback`.
  *
  * @tparam A
  ...
  * @since 0.10.0
  */
final class CallbackTo[+A] /*private[react]*/ (
    private[CallbackTo] val trampoline: Trampoline[A]
) extends AnyVal { self =>

  /** Executes this callback, on the current thread, right now, blocking until
    * complete. Exceptions will not be thrown, not caught. Use
    * [[CallbackTo#attempt]] to catch exceptions.
    *
    * Typically, callbacks are passed to scalajs-react and you're expected not
    * to call this method yourself. Generally speaking, the only time you should
    * call this method is in some other non-React code's async callback. Inside
    * an AJAX callback is a common example. Even for those cases though, you can
    * avoid calling this by getting direct access instead of callback-based
    * access to your component; see the online WebSockets example:
    * https://japgolly.github.io/scalajs-react/#examples/websockets
    *
    * While it's technically safe to call [[CallbackTo#runNow]] inside the body
    * of another [[Callback]], it's better to just use combinators like
    * [[CallbackTo#flatMap]] or even a Scala for-comprehension.
    */
  inline def runNow(): A =
    trampoline.run
  ...
  /** Alias for `map`. */
  inline def |>[B](inline f: A => B)(using
      ev: MapGuard[B]
  ): CallbackTo[ev.Out] =
    map(f)
  ...
  /** Sequence a callback to run after this, discarding any value produced by
    * this.
    */
  ...
  /** Alias for `>>`.
    *
    * Where `>>` is often associated with Monads, `*>` is often associated with
    * Applicatives.
    */
  ...
  private[react] def _debounceMs(
      delayMs: Long
  )(implicit timer: Timer): Callback = {
  ...
  inline def thenRun[B](inline runNext: B)(using
      ev: MapGuard[B]
  ): CallbackTo[ev.Out] =
  ...
  /** Turns this into an [[AsyncCallback]] that runs whenever/wherever it's
    * called; `setTimeout` isn't used.
    *
  ...
  def withDuration[B](
      f: (A, FiniteDuration) => CallbackTo[B]
  ): CallbackTo[B] = {
  ...
  def logDuration(fmt: FiniteDuration => String): CallbackTo[A] =
    withDuration((a, d) => Callback.log(fmt(d)) ret a)
    ...
    map[A](a =>
      if f(a) then a
        ...
        throw new NoSuchElementException(
          "CallbackTo.withFilter predicate is not satisfied"
        )
    )
}
