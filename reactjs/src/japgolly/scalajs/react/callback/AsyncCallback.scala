...
object AsyncCallback {
  ...
  def init[A, B](
      f: (Try[B] => Callback) => CallbackTo[A]
  ): CallbackTo[(A, AsyncCallback[B])] =
  ...
  /** Create an AsyncCallback and separately provide the completion function.
    *
    * This is like Scala's promise, not the JS promise which is more like
    * Scala's Future.
    */
  ...
  private[react] def firstS[A](
      f: State => (Try[A] => Callback) => Callback
  ): AsyncCallback[A] =
    new AsyncCallback(s =>
      g =>
        CallbackTo {
  ...
  /** Callback that isn't created until the first time it is used, after which
    * it is reused.
    */
  ...
  /** Traverse stdlib T over AsyncCallback. Distribute AsyncCallback over stdlib
    * T.
    */
  def traverse[T[X] <: Iterable[X], A, B](ta: => T[A])(
      f: A => AsyncCallback[B]
  )(using cbf: BuildFrom[T[A], B, T[B]]): AsyncCallback[T[B]] =
  ...
  /** Sequence stdlib T over AsyncCallback. Co-sequence AsyncCallback over
    * stdlib T.
    */
  def sequence[T[X] <: Iterable[X], A](tca: => T[AsyncCallback[A]])(using
      cbf: BuildFrom[T[AsyncCallback[A]], A, T[A]]
  ): AsyncCallback[T[A]] =
  ...
  def traverseOption[A, B](
      oa: => Option[A]
  )(f: A => AsyncCallback[B]): AsyncCallback[Option[B]] =
  ...
  /** Sequence Option over AsyncCallback. Co-sequence AsyncCallback over Option.
    */
  def sequenceOption[A](
      oca: => Option[AsyncCallback[A]]
  ): AsyncCallback[Option[A]] =
    traverseOption(oca)(identityFn)

  /** Same as [[traverse()]] except avoids combining return values. */
  def traverse_[T[X] <: Iterable[X], A, B](
      ta: => T[A]
  )(f: A => AsyncCallback[B]): AsyncCallback[Unit] =
    AsyncCallback.suspend {
  ...
  def fromFuture[A](
      fa: => Future[A]
  )(using ec: ExecutionContext): AsyncCallback[A] =
    AsyncCallback(f =>
      Callback {
        val future = fa
        future.value match {
  ...
  def fromCallbackToFuture[A](c: CallbackTo[Future[A]])(using
      ec: ExecutionContext
  ): AsyncCallback[A] =
  ...
  def fromCallbackToJsPromise[A](
      c: CallbackTo[js.Promise[A]]
  ): AsyncCallback[A] =
    c.asAsyncCallback.flatMap(fromJsPromise(_))

  /** Not literally tail-recursive because AsyncCallback is continuation-based,
    * but this utility in this shape may still be useful.
    */
  def tailrec[A, B](
      a: A
  ...
  private[react] inline def debounce[A](
      inline delayMs: Long,
      inline self: AsyncCallback[A]
  )(implicit timer: Timer): AsyncCallback[A] =
    if (delayMs <= 0)
      self
    else {
  ...
  private inline def _debounce[A, B](
      inline delayMs: Long,
      inline f: A => AsyncCallback[B]
  )(using timer: Timer): A => AsyncCallback[B] = {
  ...
  /** Creates an debounce boundary.
    *
    * Save it as a `val` somewhere because it relies on internal state that must
    * be reused.
    */
  ...
  final case class Forked[+A](
      await: AsyncCallback[A],
  ...
  /** A synchronisation aid that allows you to wait for another async process to
    * complete.
    */
  ...
  final class CountDownLatch(count: Int, barrier: Barrier) {
    ...
    /** Decrements the count of the latch, releasing all waiting computations if
  ...
  /** A synchronization aid that allows you to wait until a set of async
    * processes completes.
    */
  ...
  final class Mutex private[AsyncCallback] () {
    ...
    /** Wrap a [[AsyncCallback]] so that it executes in the mutex.
      *
      * Note: THIS IS NOT RE-ENTRANT. Calling this from within the mutex will
      * block.
      */
  ...
  final class ReadWriteMutex private[AsyncCallback] () {
    ...
    /** Wrap a [[AsyncCallback]] so that it executes in the write-mutex. There
      * can only be one writer active at one time.
      *
      * Note: THIS IS NOT RE-ENTRANT. Calling this from within the read or write
      * mutex will block.
      */
    ...
    /** Wrap a [[AsyncCallback]] so that it executes in the read-mutex. There
      * can be many readers active at one time.
      *
  ...
  final class Ref[A] private[AsyncCallback] (
      atomicReads: Boolean,
    ...
    /** Synchronously return whatever value is currently stored. (Ignores
      * atomicity).
      */
  ...
  extension [A, B](self: AsyncCallback[A => B]) {
    ...
    /** Function distribution. See `AsyncCallback.liftTraverse(f).id` for the
      * dual.
      */
...
/** Pure asynchronous callback.
  *
  * You can think of this as being similar to using `Future` - you can use it in
  * for-comprehensions the same way - except `AsyncCallback` is pure and doesn't
  * need an `ExecutionContext`.
  *
  * When combining instances, it's good to know which methods are sequential and
  * which are parallel (or at least concurrent).
  *
  ...
  *
  * In order to actually run this, or get it into a shape in which in can be
  ...
  *   - [[unsafeToJsPromise()]]
  *
  * A good example is the [Ajax 2
  * demo](https://japgolly.github.io/scalajs-react/#examples/ajax-2).
  *
  * @tparam A
  *   The type of data asynchronously produced on success.
  */
final class AsyncCallback[+A] private[AsyncCallback] (
    val underlyingRepr: AsyncCallback.UnderlyingRepr[A]
) extends AnyVal { self =>
  ...
  /** Sequence the argument a callback to run after this, discarding any value
    * produced by this.
    */
  def >>[B](runNext: AsyncCallback[B]): AsyncCallback[B] =
    flatMap(_ => runNext)

  /** Sequence a callback to run before this, discarding any value produced by
    * it.
    */
  ...
  /** When the callback result becomes available, perform a given side-effect
    * with it.
    */
      ...
      cc =>
        s.cancelably(CallbackTo {
          ...
          val respond = Callback {
            if (r.isEmpty) {
              (ra, rb) match {
                case (Some(Success(a)), Some(Success(b))) =>
                  r = Some(catchAll(f(a, b)))
                case (Some(Failure(e)), _) => r = Some(Failure(e))
                case (_, Some(Failure(e))) => r = Some(Failure(e))
                case (Some(Success(_)), None) | (None, Some(Success(_))) |
                    (None, None) =>
                  ()
              }
  ...
  /** Start both this and the given callback at once and when both have
    * completed successfully, discard the value produced by this.
    */
  def *>[B](next: AsyncCallback[B]): AsyncCallback[B] =
    zipWith(next)((_, b) => b)

  /** Start both this and the given callback at once and when both have
    * completed successfully, discard the value produced by the given callback.
    */
  ...
  /** Discard the value produced by this callback.
    *
    * This method allows you to be explicit about the type you're discarding
    * (which may change in future).
    */
  inline def voidExplicit[B](using inline ev: A <:< B): AsyncCallback[Unit] =
    void

  /** Wraps this callback in a try-catch and returns either the result or the
  ...
  /** If this completes successfully, discard the result. If any exception
    * occurs, call `printStackTrace` and continue.
    *
    * @since 2.0.0
    */
  ...
  def handleError[AA >: A](
      f: Throwable => AsyncCallback[AA]
  ): AsyncCallback[AA] =
    new AsyncCallback(s =>
      g =>
        underlyingRepr(s) {
  ...
  def maybeHandleError[AA >: A](
      f: PartialFunction[Throwable, AsyncCallback[AA]]
  ): AsyncCallback[AA] =
    new AsyncCallback(s =>
      g =>
        underlyingRepr(s) {
  ...
  /** Return a version of this callback that will only execute once, and reuse
    * the result for all other invocations.
    */
  def memo(): AsyncCallback[A] = {
    ...
    AsyncCallback.suspend {
      result.getOrElse {
        val first = attemptTry.flatMap(t =>
          AsyncCallback.suspend(set(AsyncCallback.const(t)))
        )
  ...
  def when(cond: => Boolean): AsyncCallback[Option[A]] =
    new AsyncCallback(s =>
      f =>
        if (cond) underlyingRepr(s)(ea => f(ea.map(Some(_))))
        else f(Success(None))
    )
  ...
  /** Conditional execution of this callback. Discards the result. Reverse of
    * [[when_()]].
    *
    * @param cond
  ...
  /** Limits the number of invocations in a given amount of time.
    ...
    *   Some if invocation was allowed, None if rejected/rate-limited
    */
  inline def rateLimit(
      inline window: FiniteDuration
  ): AsyncCallback[Option[A]] =
    rateLimitMs(window.toMillis)
  ...
  private[react] inline def _rateLimitMs(
      inline windowMs: Long,
      inline maxPerWindow: Int,
      inline clock: RateLimit.Clock
  ): AsyncCallback[Option[A]] =
  ...
  /** Creates an debounce boundary over the underlying computation.
    *
    * Save the result of this as a `val` somewhere because it relies on internal
    * state that must be reused.
    */
  ...
  def logAround(message: Any, optionalParams: Any*): AsyncCallback[A] = {
    def log(prefix: String) =
      Callback.log(prefix + message, optionalParams: _*).asAsyncCallback
  ...
  /** Limit the amount of time you're prepared to wait for a computation.
    *
    * Note: there's no built-in concept of cancellation here. If your procedure
    * doesn't finish in time, this will return `None` when the time limit is
    * reached however, the underlying procedure will become orphaned and
    * continue to run in the background until complete.
    */
  ...
  inline def setInterval(
      inline dur: Duration
  ): CallbackTo[Callback.SetIntervalResult] =
    toCallback.setInterval(dur)

  /** Schedule for repeated execution every `dur`. */
  inline def setInterval(
      inline dur: FiniteDuration
  ): CallbackTo[Callback.SetIntervalResult] =
    toCallback.setInterval(dur)

  /** Schedule for repeated execution every x milliseconds. */
  inline def setIntervalMs(
      inline milliseconds: Double
  ): CallbackTo[Callback.SetIntervalResult] =
  ...
  inline def setTimeout(
      inline dur: Duration
  ): CallbackTo[Callback.SetTimeoutResult] =
  ...
  /** Schedule for execution after x milliseconds.
    ...
    */
  inline def setTimeoutMs(
      inline milliseconds: Double
  ): CallbackTo[Callback.SetTimeoutResult] =
    toCallback.setTimeoutMs(milliseconds)

  /** Wraps this callback in a `try-finally` block and runs the given callback
    * in the `finally` clause, after the current callback completes, be it in
    * error or success.
    */
  ...
  /** Start both this and the given callback at once use the first result to
    * become available, regardless of whether it's a success or failure.
    */
  def race[B](that: AsyncCallback[B]): AsyncCallback[Either[A, B]] =
    AsyncCallback.firstS(s =>
      f =>
  ...
  def toCallback: Callback =
    AsyncCallback.newState.flatMap(
      underlyingRepr(_)(AsyncCallback.defaultCompleteWith)
    )
  ...
  /** Returns a synchronous [[Callback]] that when run, returns the result on
    * the [[Right]] if this [[AsyncCallback]] is actually synchronous, else
    * returns a new [[AsyncCallback]] on the [[Left]] that waits for the async
  ...
  /** THIS IS VERY CONVENIENT IN UNIT TESTS BUT DO NOT RUN THIS IN PRODUCTION
    * CODE.
    *
    * Executes this now, expecting and returning a synchronous result. If there
    * are any asynchronous computations this will throw an exception.
    */
  ...
  def handleErrorSync[AA >: A](
      f: Throwable => CallbackTo[AA]
  ): AsyncCallback[AA] =
    handleError(f(_).asAsyncCallback)

  def maybeHandleErrorSync[AA >: A](
      f: PartialFunction[Throwable, CallbackTo[AA]]
  ): AsyncCallback[AA] =
  ...
  def fork: CallbackTo[AsyncCallback.Forked[A]] =
    AsyncCallback.promise[A].flatMap { case (promise, completePromise) =>
      ...
      val runInBg = self.attemptTry
        .finallyRunSync(markComplete)
        .flatMapSync(completePromise)
        .fork_
  ...
  /** Runs this async computation in the background.
    *
    * Unlike [[fork]] this returns nothing, meaning this is like
    * fire-and-forget.
    */
  ...
  /** Runs this async computation in the background.
    *
    * Unlike [[fork_]] this returns an `AsyncCallback[Unit]` instead of a
    * `Callback`.
    */
  ...
  def withDuration[B](
      f: (A, FiniteDuration) => AsyncCallback[B]
  ): AsyncCallback[B] = {
    val nowMS: AsyncCallback[Long] =
      CallbackTo.currentTimeMillis.asAsyncCallback
    for {
      s <- nowMS
      a <- self
      e <- nowMS
  ...
  def logDuration(fmt: FiniteDuration => String): AsyncCallback[A] =
    withDuration((a, d) => Callback.log(fmt(d)).asAsyncCallback ret a)
    ...
    map[A](a =>
      if f(a) then a
        ...
        throw new NoSuchElementException(
          "AsyncCallback.withFilter predicate is not satisfied"
        )
    )
