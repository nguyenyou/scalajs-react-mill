...
final class SetStateFn[F[_], A[_], S](
    underlyingFn: (Option[S], F[Unit]) => F[Unit]
)(implicit FF: UnsafeSync[F], AA: Async[A])
    extends AbstractFunction2[Option[S], F[Unit], F[Unit]]
    with StateAccess.SetState[F, A, S] {
  ...
  override def withEffect[G[_]](implicit G: UnsafeSync[G]) =
    G.subst[F, WithEffect](this)(
      new SetStateFn(G.transSyncFn2C(underlyingFn)(F))(G, A)
    )(F)
  ...
  override def setStateOption[G[_]](newState: Option[S], callback: => G[Unit])(
  ...
  def contramap[B](f: B => S): SetStateFn[F, A, B] =
    SetStateFn[F, A, B]((a, cb) => underlyingFn(a map f, cb))(using F, A)
...
object SetStateFn {
  def apply[F[_]: UnsafeSync, A[_]: Async, S](
      f: (Option[S], F[Unit]) => F[Unit]
  ): SetStateFn[F, A, S] =
    new SetStateFn(f)
}
...
final class ModStateFn[F[_], A[_], S](
    underlyingFn: (S => Option[S], F[Unit]) => F[Unit]
)(implicit FF: UnsafeSync[F], AA: Async[A])
    extends AbstractFunction2[S => Option[S], F[Unit], F[Unit]]
    with StateAccess.ModState[F, A, S] {
  ...
  override def withEffect[G[_]](implicit G: UnsafeSync[G]) =
    G.subst[F, WithEffect](this)(
      new ModStateFn(G.transSyncFn2C(underlyingFn)(F))(G, A)
    )(F)
  ...
  /** @param callback Executed regardless of whether state is changed. */
  override def modStateOption[G[_]](f: S => Option[S], callback: => G[Unit])(
      implicit G: Dispatch[G]
  ): F[Unit] =
  ...
  def toModStateWithPropsFn[P](p: P): ModStateWithPropsFn[F, A, P, S] =
    ModStateWithPropsFn[F, A, P, S]((f, cb) => underlyingFn(f(_, p), cb))(using
      F,
      A
    )

  def toSetStateFn: SetStateFn[F, A, S] =
    SetStateFn[F, A, S]((s, cb) => underlyingFn(_ => s, cb))(using F, A)

  def xmapState[B](f: S => B)(g: B => S): ModStateFn[F, A, B] =
    ModStateFn[F, A, B]((m, cb) => underlyingFn(s => m(f(s)) map g, cb))(using
      F,
      A
    )
}
...
final class ModStateWithPropsFn[F[_], A[_], P, S](
    underlyingFn: ((S, P) => Option[S], F[Unit]) => F[Unit]
)(implicit FF: UnsafeSync[F], AA: Async[A])
    extends AbstractFunction2[(S, P) => Option[S], F[Unit], F[Unit]]
    with StateAccess.ModStateWithProps[F, A, P, S] {
  ...
  override def withEffect[G[_]](implicit G: UnsafeSync[G]) =
    G.subst[F, WithEffect](this)(
      new ModStateWithPropsFn(G.transSyncFn2C(underlyingFn)(F))(G, A)
    )(F)

  override def withAsyncEffect[G[_]](implicit G: Async[G]) =
    G.subst[A, WithAsyncEffect](this)(
      new ModStateWithPropsFn(underlyingFn)(F, G)
    )(A)
  ...
  def toModStateFn: ModStateFn[F, A, S] =
    ModStateFn[F, A, S]((f, cb) => underlyingFn((s, _) => f(s), cb))(using F, A)

  def toSetStateFn: SetStateFn[F, A, S] =
    SetStateFn[F, A, S]((s, cb) => underlyingFn((_, _) => s, cb))(using F, A)

  def mapProps[B](f: P => B): ModStateWithPropsFn[F, A, B, S] =
    ModStateWithPropsFn[F, A, B, S]((m, cb) =>
      underlyingFn((s, p) => m(s, f(p)), cb)
    )(using F, A)
  ...
  def xmapState[B](f: S => B)(g: B => S): ModStateWithPropsFn[F, A, P, B] =
    ModStateWithPropsFn[F, A, P, B]((m, cb) =>
      underlyingFn((s, p) => m(f(s), p) map g, cb)
    )(using F, A)
}

object ModStateWithPropsFn {
  def apply[F[_]: UnsafeSync, A[_]: Async, P, S](
      f: ((S, P) => Option[S], F[Unit]) => F[Unit]
  ): ModStateWithPropsFn[F, A, P, S] =
