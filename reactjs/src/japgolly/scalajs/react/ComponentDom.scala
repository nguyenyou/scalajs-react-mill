...
object ComponentDom {
  ...
  def findDOMNode(
      a: dom.Element | facade.React.ComponentUntyped
  ): ComponentDom = {
  ...
  sealed trait Mounted extends ComponentDom {
    ...
    final def asElement(): dom.Element =
      this match {
        case Element(e) => e
        case x =>
          throw new RuntimeException(s"Expected a dom.Element; got ${x.raw}")
      }
      ...
      node match {
        ...
        case _ =>
          throw new RuntimeException(
            s"The .fold method is now deprecated and doesn't support non-Text, non-Element nodes"
          )
      }
  }
