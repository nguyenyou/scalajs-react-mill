...
trait ReactEventTypes {
  ...
  final type ReactAnimationEventFrom[+N <: dom.Node] =
    facade.SyntheticAnimationEvent[N]
  final type ReactClipboardEventFrom[+N <: dom.Node] =
    facade.SyntheticClipboardEvent[N]
  final type ReactCompositionEventFrom[+N <: dom.Node] =
    facade.SyntheticCompositionEvent[N]
  ...
  final type ReactKeyboardEventFrom[+N <: dom.Node] =
    facade.SyntheticKeyboardEvent[N]
  final type ReactMouseEventFrom[+N <: dom.Node] = facade.SyntheticMouseEvent[N]
  final type ReactPointerEventFrom[+N <: dom.Node] =
    facade.SyntheticPointerEvent[N]
  final type ReactTouchEventFrom[+N <: dom.Node] = facade.SyntheticTouchEvent[N]
  final type ReactTransitionEventFrom[+N <: dom.Node] =
    facade.SyntheticTransitionEvent[N]
  ...
  final type ReactCompositionEventFromHtml =
    ReactCompositionEventFrom[html.Element]
  ...
  final type ReactTransitionEventFromHtml =
    ReactTransitionEventFrom[html.Element]
  ...
  final type ReactCompositionEventFromInput =
    ReactCompositionEventFrom[html.Input]
  ...
  final type ReactTransitionEventFromInput =
    ReactTransitionEventFrom[html.Input]
  ...
  final type ReactAnimationEventFromTextArea =
    ReactAnimationEventFrom[html.TextArea]
  final type ReactClipboardEventFromTextArea =
    ReactClipboardEventFrom[html.TextArea]
  final type ReactCompositionEventFromTextArea =
    ReactCompositionEventFrom[html.TextArea]
  ...
  final type ReactKeyboardEventFromTextArea =
    ReactKeyboardEventFrom[html.TextArea]
  final type ReactMouseEventFromTextArea = ReactMouseEventFrom[html.TextArea]
  final type ReactPointerEventFromTextArea =
    ReactPointerEventFrom[html.TextArea]
  final type ReactTouchEventFromTextArea = ReactTouchEventFrom[html.TextArea]
  final type ReactTransitionEventFromTextArea =
    ReactTransitionEventFrom[html.TextArea]
  ...
  @inline final implicit def toReactExt_DomEvent(
      e: dom.Event
  ): ReactExt_DomEvent[D.Sync] =
    new ReactExt_DomEvent(e)(D.Sync)

  @inline final implicit def toReactExt_ReactEvent[E <: ReactEvent](
      e: E
  ): ReactExt_ReactEvent[D.Sync, E] =
    new ReactExt_ReactEvent(e)(D.Sync)

  @inline final implicit def toReactExt_ReactKeyboardEvent[N <: dom.Node](
      e: ReactKeyboardEventFrom[N]
  ): ReactExt_ReactKeyboardEvent[N] =
    new ReactExt_ReactKeyboardEvent(e)
}

final class ReactExt_DomEvent[F[_]](private val e: dom.Event)(implicit
    F: Sync[F]
) {

  /** Stops the default action of an element from happening. For example:
  ...
  /** Stops the bubbling of an event to parent elements, preventing any parent
...
final class ReactExt_ReactEvent[F[_], E <: facade.SyntheticEvent[dom.Node]](
    private val e: E
)(implicit F: Sync[F]) {
  ...
  /** If you want to access the event properties in an asynchronous way (eg. in
    * a `modState(…)` function), React will have recycled the event by the time
    * the asynchronous call executes.
    *
    * This convenience function extracts a value from the event synchronously
    * (i.e. now!) and so that it is available to the asynchronous code.
    */
...
final class ReactExt_ReactKeyboardEvent[N <: dom.Node](
    private val e: facade.SyntheticKeyboardEvent[N]
) extends AnyVal {

  /** Checks the state of all pressed modifier keys.
    *
    * `e.pressedModifierKeys()` returns `true` if no modifier keys are currently
    * pressed.
    *
    * `e.pressedModifierKeys(altKey = true)` returns `true` if alt is the only
    * modifier key currently pressed.
    */
  def pressedModifierKeys(
