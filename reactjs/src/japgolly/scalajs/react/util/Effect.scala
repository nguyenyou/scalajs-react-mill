...
object Effect extends EffectFallbacks {
  ...
  trait UnsafeSync[F[_]] extends Dispatch[F] {
    ...
    final def subst[G[_], X[_[_]]](xg: X[G])(xf: => X[F])(implicit
        g: UnsafeSync[G]
    ): X[F] =
    ...
    final def transSyncFn1[G[_], A, B](
        f: A => G[B]
    )(implicit g: UnsafeSync[G]): A => F[B] =
      if (this eq g)
        f.asInstanceOf[A => F[B]]
      else
        a => delay(g.runSync(f(a)))

    final def transSyncFn2[G[_], A, B, C](
        f: (A, B) => G[C]
    )(implicit g: UnsafeSync[G]): (A, B) => F[C] =
      if (this eq g)
        f.asInstanceOf[(A, B) => F[C]]
      else
        (a, b) => delay(g.runSync(f(a, b)))

    final def transSyncFn2C[G[_], A, B, C](
        f: (A, G[B]) => G[C]
    )(implicit g: UnsafeSync[G]): (A, F[B]) => F[C] =
      if (this eq g)
        f.asInstanceOf[(A, F[B]) => F[C]]
      else
        (a, gb) => delay(g.runSync(f(a, g.transSync(gb)(this))))

    final def transDispatch[G[_]](
        f: => G[Unit]
    ...
    final def transDispatchFn1[G[_], A](
        f: A => G[Unit]
    )(implicit g: Dispatch[G]): A => F[Unit] =
      if (this eq g)
        f.asInstanceOf[A => F[Unit]]
      else
        a => delay(g.dispatch(f(a)))

    final def transDispatchFn3[G[_], A, B, C](
        f: (A, B, C) => G[Unit]
    )(implicit g: Dispatch[G]): (A, B, C) => F[Unit] =
      if (this eq g)
  ...
  object UnsafeSync {
    ...
    trait WithDefaults[F[_]]
        extends UnsafeSync[F]
        with Dispatch.WithDefaults[F] {
    ...
    implicit lazy val id: UnsafeSync[Id] = new WithDefaults[Id] {
      ...
      override def finallyRun[A, B](a: => A, runFinally: => B) = try a
      finally runFinally
      ...
      override def traverse_[A, B](as: => Iterable[A])(f: A => B) =
        as.foreach(f)
  ...
  object Sync {
    ...
    trait WithDefaults[F[_]]
        extends WithDefaultDispatch[F]
        with UnsafeSync.WithDefaults[F] {
      ...
      /** Wraps this callback in a `try-finally` block and runs the given
        * callback in the `finally` clause, after the current callback
        * completes, be it in error or success.
        */
      override def finallyRun[A, B](fa: => F[A], runFinally: => F[B]): F[A] =
        delay {
          try runSync(fa)
          finally runSync(runFinally)
        }
      ...
      override def handleError[A, AA >: A](
          fa: => F[A]
      )(f: Throwable => F[AA]): F[AA] =
        delay[AA](
          try
            runSync(fa)
          catch {
