...
final class ConsoleHijack(val config: ConsoleHijack.Config) {
  ...
  /** Install these hijacks into `console`, execute the given procedure, then
    * restore `console` to the state that it was before this is called.
    */
  def apply[A](a: => A): A = {
    ...
    try {
      _install(undo)
      a
    } finally undo.foreach(_())
  }

  /** Install these hijacks into `console`.
    *
    * @return
    *   A procedure to restore `console` to the state that it was before this is
    *   called.
    */
  def install(): () => Unit = {
    val undo = new js.Array[() => Unit]
...
object ConsoleHijack {
    ...
    lazy val msg: String =
      if (args.isEmpty)
        ...
        args.head match {
          case fmt: String =>
            try
              Internals.vsprintf(fmt, js.Array(args.tail*))
            catch {
  ...
  private object Internals {
    ...
    def mkHandler(f: Handler, orig: JsVarargsFn): JsVarargsFn = { args =>
      val i = Intercept(args, () => orig(args*))
      f(i)
    }
  }
}
