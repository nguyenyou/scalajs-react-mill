...
object UpdateSnapshot {
  ...
  @implicitNotFound(
    "You can only specify getSnapshotBeforeUpdate once, and it has to be before " +
  ...
  inline given safetyProof[U <: UpdateSnapshot](using
      inline ev: U =:= UpdateSnapshot.None
  ): SafetyProof[U] =
    null
}
